...
public class DoctorController {
    ...
    @PostMapping
    @PreAuthorize("hasRole('PATIENT')")
    public ResponseEntity<ApiResponse> createDoctor(@Valid @RequestBody DoctorRegistrationRequestDto request) {
        doctorService.createDoctor(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(new ApiResponse(HttpStatus.CREATED.value(), "Doctor created successfully"));
    }
}
